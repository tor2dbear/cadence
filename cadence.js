...
function buildStyleDictionary(){
  ...
  }; if(it.purpose) t.comment=it.purpose;   // Style Dictionary's standard metadata field
...
function buildRationale(){
  ...
  L.push("## Intents", "");
  intents.forEach(it=>{ const r=resolve(it); L.push(`- **${it.name}**${it.purpose?` — ${it.purpose}`:""} · ${r.d} · ${r.eLabel}`); });
  L.push("");
  L.push("## Scorecard", "");
